...
export default function MainPage() {
  ...
  const connectSsh = useCallback(() => {
    ...
    ws.onmessage = (event) => {
      ...
      if (msg.type === "data" && msg.data) {
        ...
        setStatus(s);
        if (s === "connected") { setActiveConnName(conn.name); xtermRef.current?.writeln(`\x1b[32m✓ Connected to ${conn.name}\x1b[0m`); }
        else if (s === "disconnected") { setActiveConnName(""); xtermRef.current?.writeln("\x1b[33m⚡ Disconnected\x1b[0m"); agentStopRef.current = true; }
      } else if (msg.type === "error" && msg.data) {
        xtermRef.current?.writeln(`\x1b[31m✗ Error: ${msg.data}\x1b[0m`);
        setStatus("disconnected"); agentStopRef.current = true;
      }
    };
    ws.onclose = () => { setStatus("disconnected"); setActiveConnName(""); agentStopRef.current = true; };
    ws.onerror = () => { setStatus("disconnected"); xtermRef.current?.writeln("\x1b[31m✗ WebSocket error\x1b[0m"); };
  }, [selectedConnId, connections.data]);

  const disconnect = useCallback(() => {
    agentStopRef.current = true;
    if (wsRef.current) { wsRef.current.send(JSON.stringify({ type: "disconnect" })); wsRef.current.close(); wsRef.current = null; }
          ...
          sendMessage.mutate(
            { data: { message: VOICE_SYSTEM_NOTE + transcript, terminalContext: termOutputRef.current.slice(-2000) } },
            {
              onSuccess: (resp) => {
                ...
                if (liveModeRef.current && resp.message) {
  ...
  const toggleLiveMode = useCallback(() => {
    ...
    if (liveModeRef.current) {
      ...
      // Switch to chat tab on mobile so user sees status
      if (isMobile) setMobileTab("chat");
      setTimeout(startListening, 300);
    }
  }, [agentRunning, isMobile, startListening, stopListening, toast]);

  useEffect(() => {
    return () => { liveModeRef.current = false; stopListening(); window.speechSynthesis.cancel(); };
  }, [stopListening]);
    ...
    <div className="flex flex-col h-full overflow-hidden" data-testid="main-page">
      ...
      <div className="flex items-center gap-2 px-3 h-12 border-b border-border bg-card shrink-0">
        ...
        <span className="text-xs font-mono text-muted-foreground truncate flex-1 min-w-0" data-testid="text-connection-status">
          {status === "connected" ? activeConnName : status === "connecting" ? "Connecting..." : "Disconnected"}
        </span>
      ...
      <div className="flex flex-1 overflow-hidden min-h-0">
        ...
        <div className={cn(
          ...
          // Mobile: full width when terminal tab active, hidden otherwise
          ...
          {status === "connected" && (mobileTab === "terminal" || !isMobile) && (
            <div className="flex overflow-x-auto gap-1 p-1.5 border-t border-border bg-card shrink-0" data-testid="mobile-keyboard">
              ...
              {/* Snippet picker on mobile inside keyboard bar */}
        ...
        <div className={cn(
          ...
          <div ref={chatScrollRef} className="flex-1 overflow-y-auto p-3 space-y-3 min-h-0" data-testid="chat-messages">
            ...
            {chatHistory.data?.map((msg) => {
              ...
              return (
                <div key={msg.id} className={cn("flex gap-2", msg.role === "user" ? "justify-end" : "justify-start")} data-testid={`chat-message-${msg.id}`}>
                  {msg.role === "assistant" && (
                  ...
                  <div className={cn(
                    ...
                    msg.role === "user" ? "bg-primary text-primary-foreground" : "bg-card border border-card-border text-foreground"
                  )}>
          ...
          {liveMode && (
            <div className="shrink-0 p-3 border-t border-border bg-card" data-testid="live-mode-footer">
              <button
                ...
                onClick={() => {
                  if (isSpeaking) { window.speechSynthesis.cancel(); isSpeakingRef.current = false; setIsSpeaking(false); setTimeout(startListening, 200); }
